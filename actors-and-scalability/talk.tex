...
\usepackage{listings}
\lstset{basicstyle=\footnotesize,language=Scala}
...
\begin{frame}
  ...
  \begin{columns}
    ...
    \centering
    \includegraphics[scale=0.28]{img/europe-jk}
  \end{columns}
\end{frame}
...
\begin{frame}
  \frametitle{Ziel: Optimale Auslastung von Ressourcen}
  \begin{itemize}
  ...
  \item \textbf{Heute:} Vernetzte Systeme, Cloud
    ...
    \item Virtuell: Vom schlanken Container bis zum voll virtualisierten OS
    \end{itemize}
  \end{itemize}
  \pause
  ...
  \begin{block}{Relevante Metriken}
    \begin{itemize}
    \item Verständlichkeit / Benutzerfreundlichkeit
    \item Effizienz
...
\begin{frame}
  \frametitle{Zwei Modelle: Threads vs.\ Actors\footnote{\mycite{Hewitt, Bishop und Steiger 1973 / Agha 1985}}}
  \begin{columns}
...
\begin{frame}
  \frametitle{Actormodell: Pro und Contra}
  \begin{itemize}
  \item Moderne CPUs haben lokale Caches an jedem Kern
    \begin{itemize}
    \item Einheitlicher, konsistenter Speicher ist unrealitisch
    \end{itemize}
  ...
  \item Actors sind logisch geschlossene Einheiten
    \begin{itemize}
    \item erleichtert dynamische Topologien
    \item erleichtert Programmierung (weniger Bugs)
    \end{itemize}
  \item Einziger Nachteil: Weniger performant aufgrund der zusätzlichen Kommunikationsebene
  \end{itemize}
  \vspace{1em}
  \pause
  \begin{block}{Fazit}
...
\begin{frame}
  \frametitle{Projektübersicht, BA Thesis 2010}
  \begin{itemize}
  \item Entwicklung eines Compilers inkl.\  Laufzeitumgebung
  \item Objektsprache: \emph{\color{greenblue}Lucid} (Datenflusssprache) \mycite{Wadge und Ashcroft 1985}
    \begin{itemize}
    \item Primärer Datentyp: {\color{sbmcyan}Streams} (Stream-Algebra)
    \end{itemize}
  \item Implementierungsprache: \emph{\color{greenblue}Scala} (JVM, objektorientiert und funktional)%
    \begin{itemize}
    \item Frontend: Parser-Kombinatoren
    \item Backend / Laufzeitumgebung: {\color{sbmcyan}nebenläufiger Datenfluss-Simulator}
    \end{itemize}
  \item Ziel: Evaluierung der Skalierbarkeit
  \item Motivation: Datenfluss-Semantik $\approx$ Actormodell
    \begin{itemize}
    \item Semantik: Programm $=$ Datenfluss-Netzwerk
    \item Vorwärts: Data-Driven, Push
    \item Rückwerts: Demand-Driven, Pull
    \end{itemize}
  \end{itemize}
\end{frame}

\begin{frame}
  \frametitle{Beispiel: Hamming Sequenz als Datenfluss-Diagramm}
  \begin{center}
    \vspace{1em}
    \includegraphics[scale=0.35]{img/hamming}
    \vspace{1em}
    \begin{align*}
      \ham &= 1, {\color{sbdcyan}2}, {\color{sbmcyan}3}, {\color{sblcyan}4}, {\color{greenblue}5}, {\color{darkred}6}, {\color{darkyellow}8}, 9, 10, 12, 15, \ldots = [2^i3^j5^k \mid \forall i j k]\\
...
\begin{frame}
  \frametitle{Implementierung der simLucid Laufzeitumgebung}
  \begin{itemize}
  \item Ein Actor pro Operator ($+$,$-$, $\times$, $\mathop{fby}$, \ldots)
  \item Jeder Actor kennt seine Kind- und Elternknoten
    \begin{itemize}
    \item Kindknoten = Operand
    \end{itemize}
  \item Eingabe: AST des Datenfluss-(Teil-)Programms
  \item Start: ein IO-Actor der mit dem User interagiert
    \begin{itemize}
    \item Anfrage von Parametern
    \item Ausgabe von Resultaten
    \end{itemize}
  \item Evaluierung: Demand-Driven
    \begin{itemize}
    \item IO-Knoten generiert Sequenz von Anfragen an den obersten Ausdrucksknoten für $t_0, t_1, t_2, \ldots$
    \end{itemize}
  \item Actornetzwerk wird bei Bedarf dynamisch aufgebaut
  \item Verwaltung eines \textit{Telefonbuchs} für mehrfach benötigte Operanden
    \begin{itemize}
    \item \emph{\color{sbmcyan}Diese Buchhaltung war etwas unbefriedigend}
    \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}
  \begin{center}
    \begin{Large}
      \hl{-- Das Actormodell heute / Cloud Deployment --}
    \end{Large}
  \end{center}
\end{frame}

\begin{frame}
  \frametitle{Wo stehen wir heute?}
  \begin{itemize}
  \item Umfangreiche Actor-Programmbibliotheken
    \begin{itemize}
    \item z.B.\ Akka und Akka Streams\footnote{\url{https://akka.io/}} von Lightbend für Java und Scala
    \end{itemize}
  \item Eingebaute Verwaltung von \emph{\color{greenblue}Actorsystemen}
    \begin{itemize}
    \item Kein manuelles Telefonbuch mehr nötig
    \item Koordinierung von großen Mengen von Actors
    \item Kontrollierter Spin-Up und Shutdown der Systeme
    \end{itemize}
  \item Reibungsloser Übergang von Intra- zu Inter-System-Kommunikation
  \item Kompatibel mit gängigen Micro-Service Frameworks
  \end{itemize}
\end{frame}
...
\begin{frame}
  \frametitle{Wo stehen wir heute?}
  \begin{itemize}
  \item \structure{Big Data:} Map-Reduce, Hadoop, Kafka, Spark
  \item \structure{Cloud Deployment:} Docker, Kubernetes, Helm, REST
  \end{itemize}
  \begin{center}
    \includegraphics[scale=0.7]{img/cloud-deploy}
  \end{center}
\end{frame}

% \begin{frame}
%   \frametitle{Moderne Anwendungen}
%   \begin{itemize}
%   \item Beispiel: Platform zur Indizierung und Analyse verteilter Datensilos
%   \item Micro-Service Design am Beispiel der Indizierung:
%     \begin{itemize}
%     \item Mehrere Services, mit quer verschalteten Actorsystemen
%     \item Datenquellen werden parallel indiziert
%     \item Horizontale Bedarfs-Skalierung inklusive
%     \item Index Ablage in einem ElasticSearch Cluster
%     \end{itemize}
%   \item Inline Stream-Analysen können via Kafka zwischengeschaltet werden
%   \item Batch Analysen werden über Spark realisiert
%   \end{itemize}
%   \vspace{2em}
%   \pause
%   \begin{center}
%     \structure{Challenge:} Demonstration der Produkt-Features
%   \end{center}
% \end{frame}

% \section{Park-Simulation}

% \begin{frame}
%   \begin{center}
%     \begin{Large}
%       \hl{-- Fallstudie: Simulation eines Freizeitparks --}
%     \end{Large}
%   \end{center}
% \end{frame}

% \begin{frame}
%   \frametitle{Projektübersicht, Freizeitpark Simulator, 2018}
%   \structure{Szenario}
%   \begin{itemize}
%   \item Fiktiver Freizeitpark
%   \item Besucher tragen Tracking-Geräte
%   \item Viele Aktionen erzeugen Event-Records
%   \item Bestimmte Probleme \emph{\color{greenblue}nur mittelbar} aus den Daten ableitbar
%   \item Komplexe JOINs und Aggregierung über mehrere Datenquellen
%   \end{itemize}
%   \pause
%   \structure{Projekt}
%   \begin{itemize}
%   \item Erzeugung von Datenquellen und Ereignissen mit Anomalien
%   \item Reine Zufallsgenerierung wird dem Problem nicht gerecht
%     \begin{itemize}
%     \item Simulation des Freizeitparkbetriebs mit Anomalie-Injektion
%     \end{itemize}
%   \end{itemize}
% \end{frame}
...
%   \begin{itemize}
%   \item Ein Actor je Besucher, ein Actor je Scanner
%   \item Jeder Besucher-Actor enthält seinen internen Zustand
%   \item Verhalten als \emph{\color{greenblue} probabilistischer NFA} modelliert
%   \item Transitionen lösen ggf.\ Interaktionen mit Scannern aus (Nachrichtenaustausch)
%   \item Scanner erzeugen bei Interaktionen Ereigniss-Records
%   \item Zwei Modi:
%     \begin{itemize}
%     \item Alle Komponenten laufen frei und lösen selbst NFA Transitionen aus
%     \item Transition von Komponenten nur bei externer Aufforderung
%     \end{itemize}
%   \item Analyse: Platform rekonstruiert die \emph{\color{greenblue}stillen Transitionen}
%   \end{itemize}
% \end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
  \item Actors liefern ein ideales Modell für nebenläufige Prozesse
    \begin{itemize}
    \item lokale Anwendungen
    \item verteilte Cloud-Anwendungen
    \end{itemize}\pause
  \item Simulatoren sind rechenintensiv und auf Nebenläufigkeit angewiesen, \ldots \pause allerdings nur während sie auch wirklich laufen\pause
    \begin{itemize}
    \item Cloud-Deployments liefern die nötige Elastizität
\end{itemize}
...
% \begin{frame}[fragile]
%   \frametitle{Implementierung der Laufzeitumgebung}
%   \structure{Operator}
%   \begin{lstlisting}
%     class Operator(op: Int -> Int -> Int) extends Actor {
%       def receive = {
%         case Request(addr, id, time) =>
%           cache(time) match {
%             case Some(v) =>  addr ! Response(id, v)
%             case None    =>
%             // query operands, register pending comp
%           }
%         case Response(id, data)      =>
%         // add data to computation with id, use op to
%         // compute result, then cache and respond
%       }
%     }
%   \end{lstlisting}
%   \structure{Nachrichtenaustausch}
%   \begin{lstlisting}
%     val addNode = new Operator(add)
%     val reqId = ...

%     addNode ! Request(self, reqId, 0)
%   \end{lstlisting}
% \end{frame}
